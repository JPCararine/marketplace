  ...
  return (
    <>
      ...
      <View className="flex-row justify-between items-center mt-4">
        ...
        <Text className="text-lg font-semibold text-gray-500">
          R$ {moneyMapper(Number(data?.value))}
        </Text>
      </View>

      <View
        className="w-full p-3 gap-3 flex-row rounded-xl mt-3"
        style={{ backgroundColor: "#D7DDF9", height: 60 }}
      >
        <View
          className="rounded-md bg-blue-dark items-center justify-center"
        ...
        >
          <Text className="font-semibold">
            {data?.views}{" "}
            {(data?.views ?? 0) > 1 ? "pessoas" : "pessoa"}
          </Text>{" "}
          {data?.views === 1
            ? "visualizou este produto nos últimos 7 dias."
            : "visualizaram este produto nos últimos 7 dias."}
        </Text>
      </View>

      <View className="justify-center mt-4 items-start gap-2">
      ...
      <View className="justify-between flex-row items-center mt-10 mb-4">
        <Text className="text-sm text-gray-500 font-semibold">
          Avaliações
        </Text>
